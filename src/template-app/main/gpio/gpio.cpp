...
esp_err_t GPIOConfig(void)
{
    ...
    gpiocfg.intr_type = GPIO_INTR_NEGEDGE;                          // Active on Negative Edge
    // 
...
void IRAM_ATTR gpio_interrupt_keypad_handler(void *args)
{
    ...
    //ESP_LOGI(TAG_GPIO, "Keypad interruption .... ");
    ...
    /*if (!button_04_intr_disabled)
    {
        ev.type = I2C_EVENT_BUTTON;
        ev.pinNumber = pinNumber;
        ev.pinState = gpio_get_level(BUTTON_04_INPUT);
        if (oledEventQueue)
        {
            button_04_intr_disabled = true;
            
            if (xQueueSendFromISR(oledEventQueue, &ev, NULL))
            {
    ...
    if (!button_02_intr_disabled)
    {
        ev.type = I2C_EVENT_BUTTON;
        ev.pinNumber = pinNumber;
        ev.pinState = gpio_get_level(BUTTON_02_INPUT);
        if (oledEventQueue)
        {
            button_02_intr_disabled = true;
    ...
    //ESP_LOGI(TAG_GPIO, "Button pressed .... ");
    //printf("DEBUG: Button 01 pressed ... \r\n");
